...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		backdrops = new FlxBackdrop(Paths.image('freeplay/grid1'), 0.2, 0.2, true, true);
		backdrops.antialiasing = ClientPrefs.globalAntialiasing;
		backdrops.alpha = 0.4;
		backdrops.x -= 35;
		add(backdrops);
		...
		if(curSelected >= songs.length) curSelected = 0;
		backdrops.color = songs[curSelected].color;
		intendedColor = backdrops.color;
		changeSelection();
		changeDiff();
	...
	function changeSelection(change:Int = 0)
	{
		...
		if(newColor != intendedColor) {
			...
			intendedColor = newColor;
			colorTween = FlxTween.color(backdrops, 1, backdrops.color, intendedColor, {
				onComplete: function(twn:FlxTween) {
					colorTween = null;
				}
			});
