...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Not pricol', 0.2], //From 0% to 19%
		['Govno', 0.4], //From 20% to 39%
		['Zalupa', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['Blya', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Norm', 0.8], //From 70% to 79%
		['Kayf', 0.9], //From 80% to 89%
		['Pricol', 1], //From 90% to 99%
		['MEGA PRICOL', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var numberMinus:Bool = false;
	...
	public var someFuckingText:FlxSpriteGroup;
	public var someShit:FlxSpriteGroup;
	...
	override public function create()
	{
		...
		volumeDownKeys = ClientPrefs.copyKey(ClientPrefs.keyBinds.get('volume_down'));
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'lucid-dreams' | 'sleeprunnin':
					curStage = 'home';
				case 'nightmare':
					curStage = 'home_but_im_lazy_to_move_charecters';
				default:
					curStage = 'stage';
			}
		}
		...
		someShit = new FlxSpriteGroup();

		var not_so_cool_text_make_new_one_later = new FlxText(350, 500, 0, "alright...", 64);
		not_so_cool_text_make_new_one_later.setFormat(Paths.font("FolksXXHeavy.ttf"), 48, FlxColor.WHITE);
		not_so_cool_text_make_new_one_later.alpha = 0;
				
		someShit.add(not_so_cool_text_make_new_one_later);
		someShit.alpha = 0;
		add(someShit);

		someFuckingText = new FlxSpriteGroup();


		var not_so_cool_text_make_new_one_later_MAYBE2 = new FlxText(300, 510, 0, "Let's try something else.", 64);
		not_so_cool_text_make_new_one_later_MAYBE2.setFormat(Paths.font("FolksXXHeavy.ttf"), 32, FlxColor.WHITE);
			
		someFuckingText.add(not_so_cool_text_make_new_one_later_MAYBE2);


		var not_so_cool_text_make_new_one_later_MAYBE3 = new FlxText(350, 550, 0, "Can you keep up?", 64);
		not_so_cool_text_make_new_one_later_MAYBE3.setFormat(Paths.font("FolksXXHeavy.ttf"), 32, FlxColor.WHITE);

		someFuckingText.add(not_so_cool_text_make_new_one_later_MAYBE3);

		someFuckingText.alpha = 0;
		add(someFuckingText);
	...
	if (curbg != null)
	{
		...
		if((FlxG.keys.anyJustPressed(volumeDownKeys))){
			numberMinus == true;
			trace(numberMinus);
		}
		...
		//	{
		//		sky.y -= Math.abs(Math.sin(elapsed * 50))/2;
		//	}	
		//	
		//if(curStage == 'home' && sky.y >= -350)
		//	{
		//		sky.y += Math.abs(Math.sin(elapsed * 50))/2;
		//	}
		...
		//var someSteps:Int = [17,20,29,31,34,36];
		var direction:Int = 1;

		if (curSong == 'chill-out')
		{
			switch(curStep) {
				case 17:
		...
		{
			if(iconP1.y < 10)
				iconP1.y = CoolUtil.coolLerp(iconP1.y, 10, Conductor.crochet / 1000 / cameraBeatSpeed);
			if(iconP2.y > 10)
				iconP2.y = CoolUtil.coolLerp(iconP2.y, 10, Conductor.crochet / 1000 / cameraBeatSpeed);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement([ 'monday_night_play','weekAyko_beat', 'weekAyko_nomiss', 'chill-out_hard', 'cheating_hard', 'too_loud']);
	...
	function goodNoteHit(note:Note):Void
	{
		...
		{	
			...
			if ((curSong == 'chill-out') && (curStep <= 130))
			{
				health -= 0.01;
				//boyfriend.playAnim('missDown', true);
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Miss Note':
						if(boyfriend.animation.getByName('miss') != null){
	...
	override function stepHit()
	{
		...
		if(curSong == "chill-out")
		{
			...
			{
				case 127:
					FlxTween.tween(someShit, {alpha: 1}, .2, {ease: FlxEase.quadInOut});
				case 142:
					FlxTween.tween(someShit, {alpha: 0}, .2, {ease: FlxEase.quadInOut});
				case 144:
					FlxTween.tween(someFuckingText, {alpha: 1}, .2, {ease: FlxEase.quadInOut});
				case 175:
					FlxTween.tween(someFuckingText, {alpha: 0}, .2, {ease: FlxEase.quadOut});
				case 192:
					FlxG.camera.flash(FlxColor.WHITE, 1);
			}
		}
	...
	override function beatHit()
	{
		...
		//	if (curBeat == 31)
		//	{
		//		var file:String = Paths.json('chill-out' + '/diaSEKS'); 
		//		if (OpenFlAssets.exists(file)) {
		//			dialogueJson = DialogueBoxPsych.parseDialogue(file);
		//		}
		//		startDialogue(dialogueJson);
		...
		if (curStage.toLowerCase() == 'home' && allowSway)
		{
			if (curBeat % 8 == 0) FlxTween.tween(skyBG, {y: -150}, Conductor.crochet * 4 / 1000, {ease: FlxEase.sineInOut});
			if (curBeat % 8 == 4) FlxTween.tween(skyBG, {y: -300}, Conductor.crochet * 4 / 1000, {ease: FlxEase.sineInOut});
		}
				...
				switch(achievementName)
				{
					case 'weekAyko_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'weekA':
									if(achievementName == 'weekAyko_nomiss') unlock = true;
							}
						}
					case 'weekAyko_beat':
						if(isStoryMode && !changedDifficulty && !usedPractice && storyPlaylist.length <= 1)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName)
							{
								case 'weekA':
									if(achievementName == 'weekAyko_beat') unlock = true;
							}
						}
					case 'chill-out_hard':
						if(curSong == 'chill-out' && endingSong) {
							unlock = true;
						}
					case 'cheating_hard':
						if(curSong == 'Cheating' && endingSong) {
							unlock = true;
						}
					case 'too_loud':
						if(numberMinus == true){
